...
using MPU6050_cb_t =
    std::function<void(std::array<float, 3> acceleration,
                       std::array<float, 3> gyro, float temperature)>;
class MPU6050_module : public Sensor_type {
public:
  std::function<void(std::vector<uint8_t>)> send_module;
  MPU6050_module(uint8_t i2c_port, uint8_t address, MPU6050_cb_t data_cb);
