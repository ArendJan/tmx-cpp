...
MPU9250_module::MPU9250_module(uint8_t i2c_port, uint8_t address,
...
void MPU9250_module::data_callback(std::vector<uint8_t> data) {
  ...
  for (int acc_idx = 0; acc_idx < 3; acc_idx++) {
    acceleration[acc_idx] = decode_float(
        data_span.subspan(acc_idx * sizeof(float)).first<sizeof(float)>());
  }

  for (int gyro_idx = 3; gyro_idx < 6; gyro_idx++) {
    gyro[gyro_idx - 3] = decode_float(
        data_span.subspan(gyro_idx * sizeof(float)).first<sizeof(float)>());
  }

  for (int mag_idx = 6; mag_idx < 9; mag_idx++) {
    magnetic_field[mag_idx - 6] = decode_float(
        data_span.subspan(mag_idx * sizeof(float)).first<sizeof(float)>());
  }

  for (int q_idx = 9; q_idx < 13; q_idx++) {
    quaternion[q_idx - 9] = decode_float(
        data_span.subspan(q_idx * sizeof(float)).first<sizeof(float)>());
  }
