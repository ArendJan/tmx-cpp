...
Shutdown_relay_module::Shutdown_relay_module(uint8_t pin,
  ...
  if (turn_off_time_sec > 200) {
    std::cerr << "Turn off time is too high for the shutdown relay module, "
                 "capping it at 200 seconds."
              << std::endl;
    this->turn_off_time_sec = 200;
  }
  if (turn_off_time_sec < 1) {
    std::cerr << "Turn off time is too low for the shutdown relay module, "
                 "setting it to 20 seconds."
              << std::endl;
...
void Shutdown_relay_module::data_callback(std::vector<uint8_t> data) {
  // should not receive any data, since this module is write only, but just in
  // case, ignore it
  std::cerr << "Warning: Shutdown relay module received data, but this module "
               "is write only. Ignoring data."
            << std::endl;
  return;
}
