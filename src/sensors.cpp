...
size_t Sensors::add_sensor(uint8_t sens_num, SENSOR_TYPE type, std::vector<uint8_t> data,
  ...
  addSensMsg.insert(addSensMsg.begin(), {1, sens_num, (uint8_t)type}); // 1 = add sensor
...
void Sensors::report_features(SENSOR_TYPE type, bool ok, std::vector<uint8_t> data) {
  #ifdef TMX_TX_DEBUG
  if (ok) {
  ...
  #endif
    std::cout << "report_features sensors: " << (int)type << " ok: " << (int)ok << std::endl;
  ...
  if(type == SENSOR_TYPE::SENSOR_MAX - 1) {
    ...
    #ifdef TMX_TX_DEBUG
    std::cout << "All Sensor checked, notifying all waiting threads" << std::endl;
    #endif
  }
}