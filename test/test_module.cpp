...
// Test the TMX class
    tmx_cpp::TMX tmx([]() {
        std::cout << "Stopping TMX..." << std::endl;
    }, "/dev/ttyACM0", 4); // Adjust the port and parse pool size as needed
...
//     assert(tmx.is_valid());
    ...
    tmx.add_digital_callback(
        10, [](uint8_t pin, uint8_t value) {
            std::cout << "Digital callback for pin " << (int)pin
